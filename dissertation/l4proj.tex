...
The sliding window average algorithm stores and averages the last n values, this would require O(n * d) memory, where n is the number of values to consider and d is the number of devices encountered. Since this is stored as a single number it only requires O(d) space. Since the exponential moving average algorithm has a smaller memory footprint, it was chosen as the algorithm for the system.
